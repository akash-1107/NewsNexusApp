...
const News = (props) => {
  ...
  const loadNewsItems = async () => {
    ...
    setLoader(false); // Set loading to false once data is loaded
  };
  ...
  return (
    <>
      ...
      >
        NewsNexus - {capitalizeFirstLetter(props.category)} Top Headlines
      </h1>

      {/* Only show the initial loading spinner */}
      {loading && <Spinner />}

      <InfiniteScroll
        dataLength={newsItems.length}
        next={fetchMoreData}
        hasMore={newsItems.length < 30}
      >
